...
export const reprType = (ty: Type): string => {
	switch (ty.kind) {
		...
		case "function":
			return ty.ret
				? `fn(${ty.params.map(reprType).join(", ")}): ${reprType(ty.ret)}`
				: `fn(${ty.params.map(reprType).join(", ")})`;
		case "union":
...
export const native = (name: string) => {
	return { kind: "native", name } satisfies Type;
};
...
export const unknown = native("unknown");

export const array = (item: Type) => {
	return { kind: "array", item } satisfies Type;
};

export const fn = (params?: Type[], ret?: Type) => {
	return {
		kind: "function",
		params: params ?? [],
		ret: ret ?? nothing,
...
export const variadic = (type: Type) => {
	return { kind: "variadic", type } satisfies Type;
};
export const generic = (name: string, bound?: Type) => {
	return { kind: "generic", name, bound } satisfies Type;
};
export const union = (...types: Type[]) => {
	return { kind: "union", types } satisfies Type;
};
...
export class TypeSolver {
	...
	satisfies(lhs: Type, rhs: Type): boolean {
		if (lhs.kind === "native" && lhs.name === "unknown") {
			/* basically evolving 'any' */
			Object.assign(lhs, rhs);
			return true;
		}

		if (lhs.kind === "native" && lhs.name === "any") return true;

		if (lhs.kind === "function" && rhs.kind === "function") {
			for (const [k, l_param] of lhs.params.entries()) {
				const r_param = rhs.params[k];

				if (!r_param || !this.satisfies(l_param, r_param)) return false;
			}

			for (const [k, r_param] of rhs.params.entries()) {
				const l_param = lhs.params[k];

				if (!l_param || !this.satisfies(l_param, r_param)) return false;
			}
			...
			if (rhs.kind === "generic") {
				...
				if (lhs.bound) {
					...
					if (!ty) throw `Undefined generic ${rhs.name} while solving`;

					if (!this.satisfies(lhs.bound, ty)) return false;
				}

				if (rhs.bound) {
					const ty = this.generics.get(lhs.name);
					if (!ty) throw `Undefined generic ${lhs.name} while solving`;

					if (!this.satisfies(rhs.bound, ty)) return false;
				}
				...
				if (ty) {
					...
					if (lhs.bound && !this.satisfies(lhs.bound, rhs)) return false;
